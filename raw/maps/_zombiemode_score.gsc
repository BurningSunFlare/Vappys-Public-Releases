...
player_add_points( event, mod, hit_location ,is_dog)
{
	...
	switch( event )
	{
		...
		case "ballistic_knife_death":
			player_points = get_zombie_death_player_points() + level.melee_bonus;
		...
		case "damage_light":
			player_points = level.dmg_automaticWeapon;
			break;
	
		case "damage":
			player_points = level.dmg_Weapon;
			break; 
...
player_add_points_kill_bonus( mod, hit_location )
{
	...
	switch( hit_location )
	{
		case "head":
		case "helmet":
			score = level.bonus_head; 
			break; 
	
		case "neck":
			score = level.bonus_neck; 
			break; 
	
		case "torso_upper":
		case "torso_lower":
			score = level.bonus_torso; 
			break;
			default:if(IsDefined(level.VanillaPlus)) score = level.bonus_torso;
	}
