...
{
	self endon( "death" );
	if(self.targetname != "boss_zombie_spawner"){
	if(IsDefined(level.bosz) || IsDefined(self.Ignore_Wadder)){
	wait 10; self zombie_entered_water();
	}
	}
	self.in_water = true;
...
{
if(IsDefined(e_player.Bought_Perk["specialty_IBB"]))
e_player setmovespeedscale(1.45);
	if( !IsDefined( e_player._in_coast_water ) && !isDefined(e_player.Bought_Perk["specialty_IBB"]) || e_player._in_coast_water == 0 && !isDefined(e_player.Bought_Perk["specialty_IBB"]))
	{
...
//	PrintLn("S: ccf pwf : " + self GetEntityNumber() + " " + self.team + " " + self.sessionteam + " " + self.sessionstate);
	...
	while( IsDefined( self ) && IsAlive( self ) )
	{
		if( frost )
		{
			...
			if( IsDefined( self._in_coast_water ) && self._in_coast_water < 1 || isDefined(self.Bought_Perk["specialty_IBB"]))
			{
...
water_watch_freeze()
{
	...
	while ( 1 )
	{
		if ( isDefined( self._in_coast_water ) && !isDefined(self.Bought_Perk["specialty_IBB"]))
		{
