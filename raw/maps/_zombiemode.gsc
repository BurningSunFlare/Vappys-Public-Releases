...
PrecacheItem("microwavegunlh_zm");
PrecacheItem("zombie_cymbal_monkey");
	//override difficulty
...
init_fx()
{
	...
	//level._effect["eye_glow"]			 		= level.eye_color;
//if( level.script == "zombie_moon" )	
		//level._effect["eye_glow"]			 		= LoadFX( "misc/fx_zombie_eye_single_blue" );
	//else
...
if(IsDefined(level.threegun_hacked) && primary_weapons_that_can_be_taken.size >= 3){
	weapon_to_take = primary_weapons_that_can_be_taken[primary_weapons_that_can_be_taken.size - 1];
			//Prevent Players From Having More Than 1 Wunder Weapon
		if (weapon_to_take == "microwavegundw_zm" || weapon_to_take == "microwavegundw_upgraded_zm")
			weapon_to_take = primary_weapons_that_can_be_taken[primary_weapons_that_can_be_taken.size - 2];
			self.weapon_to_givez = weapon_to_take;
...
} else if ( primary_weapons_that_can_be_taken.size >= 3 )
	{
		weapon_to_take = primary_weapons_that_can_be_taken[primary_weapons_that_can_be_taken.size - 1];
...
player_fake_death()
{
	...
	wait(1.5);
	if( level.automatic_restart == true )
		map_restart(0);
		else {
	players = getplayers();
	getPlayers()[0] thread Do_restart_combination();
	getPlayers()[1] thread Do_restart_combination();
	getPlayers()[2] thread Do_restart_combination();
	getPlayers()[3] thread Do_restart_combination();
		}
}

Do_restart_combination() {
	self iPrintLn("I DIED!");
for(;;) {
if(self JumpButtonPressed() && self SprintButtonPressed() && !self AttackButtonPressed() && !self FragButtonPressed() && !self useButtonPressed()) {
		self iPrintLn("I PRESSED!");

wait(1);
if(self JumpButtonPressed() && self SprintButtonPressed() && !self AttackButtonPressed() && !self FragButtonPressed() && !self useButtonPressed()) {
map_restart(0);
	self iPrintLn("I PRESSED 2");
