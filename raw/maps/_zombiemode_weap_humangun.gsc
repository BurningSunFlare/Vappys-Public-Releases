...
init()
{
level.vrzz = ::humangun_zombie_1st_hit_response;
level.vrzx = ::humangun_zombie_2nd_hit_response;
...
{
 Upgrade = player.Weapon_Upgrade_Enhance["humangun_upgraded_zm"];
 Time = Upgrade * 2 + 2;
	if( IsDefined( Upgrade ) && damageweapon == "humangun_upgraded_zm" && Upgrade > 2 )
	{
		self thread maps\zombie_coast_water::zombie_water_out();
		self.in_water = false;
		if(Upgrade == 5){
		self.Ignore_Wadder = true;
		Time += 5;
		}
		self thread maps\vapp::Friendly_Convert2( time, player, "director_zombie", "sprint1" );
