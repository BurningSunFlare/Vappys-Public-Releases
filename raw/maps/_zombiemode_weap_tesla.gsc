...
init()
{
	...
	set_zombie_var( "tesla_head_gib_chance",	100 );
...
{
	if(player GetCurrentWeapon() == "tesla_gun_upgraded_zm")
	level.DoExtra = true;
	player endon( "disconnect" );
	...
	for( i = 0; i < enemies.size; i++ )
	{
		...
		if ( tesla_end_arc_damage( arc_num + 1, player.tesla_enemies_hit, player ) )
		{			
...
tesla_end_arc_damage( arc_num, enemies_hit_num , player)
{
if( IsDefined( level.VanillaPlus ) && IsDefined( player.Weapon_Upgrade_Enhance["tesla_gun_zm"] ) ){
enemies_hit_num -= player.Weapon_Upgrade_Enhance["tesla_gun_zm"];
arc_num  -= player.Weapon_Upgrade_Enhance["tesla_gun_zm"];
}
if(isDefined(level.DoExtra)){
enemies_hit_num -= 5;
arc_num  -= 5;
