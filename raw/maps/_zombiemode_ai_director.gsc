...
director_reset_health( easy )
{
	...
	if(players.size > 1)
	{
		frackit = 100666 * num_players;
		level.frackit = frackit;
	}
	else
	{
		frackit = 0;
	...
	self.max_damage_taken = level.director_max_damage_taken * num_players - frackit;
	level.director_max_damage_taken = level.director_max_damage_taken * num_players - frackit;
	if ( is_true( easy ) )
	{
...
director_watch_damage()
{
	...
	while ( 1 )
	{
		...
		if(IsDefined(level.VanillaPlus)){
			level.BossHUD = self.dmg_taken;
			level notify("Boss_Hurt");
			//iprintln(level.BossHUD+" / "+level.director_max_damage_taken);
		}
	}
...
//	PlayFx( level._effect["director_groundhit"], self.origin );
	...
	if(IsDefined(level.VanillaPlus)){
		if(IsDefined(self.SpeedInitialized))
	wait 2;
else{
	self.SpeedInitialized = true;
	wait 4;
}
...
{
	ReturnOrigin = self.origin;
	self.zombie_tesla_hit = false;
	self.dmg_taken += 6500;
	if( IsDefined( self.is_ziplining ) )
	return;
	if ( is_true( self.leaving_level ))
	{
		return;
	...
	/*switch(randomIntRange(0,6)){
		case 1:pp_anim = %ai_zombie_boss_death_coast;iprintln("ai_zombie_boss_death_coast");break;
		case 2:pp_anim = %ai_zombie_boss_death_a_coast;iprintln("ai_zombie_boss_death_a_coast");break;
		case 3:pp_anim = %ai_zombie_boss_death_explode_coast;iprintln("ai_zombie_boss_death_explode_coast");break;
		case 4:pp_anim = %ai_zombie_boss_death_mg_coast;iprintln("ai_zombie_boss_death_mg_coast");break;
		case 5:pp_anim = %ai_zombie_boss_tesla_death_a_coast;iprintln("ai_zombie_boss_tesla_death_a_coast");break;
	}
	*/
	pp_anim = %ai_zombie_boss_death_explode_coast;
	pp_anim2 = %ai_zombie_traverse_ground_v2_walk_altA;
	wait .05;
	time = getAnimLength(pp_anim);
self director_animscripted( pp_anim, "director_calmed" );
self hide();
wait .1;
self show();
self director_animscripted( pp_anim2, "director_calmed" );
flag_set("ppyum");
wait .1;
self.shit = undefined;
self.moveplaybackrate = 1;
self director_humangun_react();
self forceTeleport( ReturnOrigin );
	self director_calmed( undefined, true );
	}
	if ( !is_true( self.is_activated ) )
	{
...
//-----------------------------------------------------------------------------------------------
// humangun calms director for a short time unpacked
// calms director permanently when packed
...
{
	self.SpeedInitialized = undefined;
	if ( is_true( self.is_activated ) )
	{
