...
main()
{
	level.Uses_IceBull = true;
	level.Ice_Origin = (-622.937, 368.359, -222.875);level.VanBull = 12;
	level.Uses_Starlight = true; level.Star_Angles = (0, 180, 0);  level.VanStar = 1;
	level.Uses_Upgrade = true;level.Upgrade_Origin = (48,3742,-545);
	...
	maps\zombie_moon_jump_pad::init();
...
moon_post_respawn_callback()
{
	...
	if( !maps\_zombiemode_equipment::limited_equipment_in_use("equip_hacker_zm") || IsDefined( level.VanillaPlus ) )
	{
...
pack_gate_poi_activate(time)
{
	...
	while( !flag( "packapunch_hacked" ) )
	{
		...
		for( i=0; i<zombies.size; i++ )
		{
			...
			else if( IsDefined( zombies[i]._poi_pack_set ) && !zombies[i]._poi_pack_set )
			{
