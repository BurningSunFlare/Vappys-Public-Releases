...
if(level.round_number > 99 || self.health > 60000){
...
// 	}
	...
	if(IsDefined(level.VanillaPlus)){
		//iprintln(self.targetname + " Spawned " +self.animname + " Health "+self.health);
		switch(self.animname){
			case "zombie":
			self thread [[level.zombie_func]]();
			if( level.round_number < 6)
			self.meleeDamage -= 15;
			self.actor_full_damage_func = ::Zomdweeb;break;
			case "quad_zombie":
			self thread [[level.quad_func]]();
			self.actor_full_damage_func = ::Zomdweeb;break;
		}
	}
}

weapons_list(weapon,health){
	if( IsDefined( self.IgnoreSpecialDamage ) )
		return;
	switch(weapon){
	...
	case "knife_ballistic_bowie_upgraded_zm":
		self.NewDam="Def";
		self.Multiplier=.3;
		self.BonusDamage = health *.1;
	break;
	
	case "sticky_grenade_zm":
	case "zombie_cymbal_monkey":
	case "frag_grenade_zm":
	case "stielhandgrenate":
	break;
	
	case "defaultweapon":
		self.NewDam="Def";
		self.Multiplier=.3;
		self.BypassCheck = true;
	case "freezegun_zm":
		self.NewDam="Frosty";
		self.Multiplier=3;
	return;
	case "freezegun_upgraded_zm":
		self.NewDam="FrostyUpgraded";
		self.Multiplier=3;
		self.BonusDamage = health /4;
	return;
	case "minigun_zm":
	if(self.Cust_Weap_Ammo["minigun_zm"] > 0)
	{
		self.BypassCheck = true;
		self.Multiplier = .15;
		self.BonusDamage = health *.02;
		if( IsDefined( self.Death_Upgraded ) )
		{
			self.Multiplier = .44;
		self.BonusDamage = health *.1;
		}
		self.NewDam="Death_Machine";
		return;
	}
	case "thundergun_zm":
	...
	case "microwavegunlh_zm":
	case "microwavegunlh_upgraded_zm":
	case "bowie_knife_zm":
	case "knife_zm":
		self.NewDam="ReturnDamage";
	return;
	case "ray_gun_zm":
		self.NewDam="Ray";
		self.Multiplier=2;
		self.BonusDamage = health /8;
	return;
	case "ray_gun_upgraded_zm":
		self.NewDam="RayUpgraded";
		self.Multiplier=1;
		self.BonusDamage = health /4;
	return;
	case "sniper_explosive_zm":
		self.NewDam="Sniplode";
		self.Multiplier=.1;
	return;
	case "sniper_explosive_upgraded_zm":
	self.NewDam="SniplodeUpgraded";
	self.Multiplier=.8;
	return;
	case "zombie_bar":
	case "ak47_zm":
	...
	case "m16_gl_upgraded_zm":
		...
		if(level.script != "zombie_moon")
			return;
		if(weapon == "mp40_zm" || weapon == "mp40_upgraded_zm" )
			self.NewDam="Specialty";
	return;
	...
	case "hk21_upgraded_zm":
		self.NewDam="Lmg";
		self.Multiplier=.35;
		self.BonusDamage = health *.15;
	return;
	}
}

Zomdweeb( inflictor, attacker, damage, flags, meansofdeath, weapon, vpoint, vdir, sHitLoc, modelIndex, psOffsetTime )
{
	self endon( "death" );
	if( self.animname == "astro_zombie" )
		return damage;
	//iprintln( " Weapon: "+weapon+ " Hit Location: " + sHitLoc );
	if(weapon == "defaultweapon")
		wait .3;
	if(IsDefined( attacker.HumanNeeds) )
	{
		attacker.HumanNeeds = undefined;
		self thread [[level.vrzz]]( undefined,attacker );
		wait .1;
		self setclientflag( level._ZOMBIE_ACTOR_FLAG_HUMANGUN_UPGRADED_HIT_RESPONSE );
		self thread [[level.vrzx]]( undefined,attacker );
		return;
	}
	if(!IsDefined(attacker.Weapon_Upgrade_Enhance[weapon]) || attacker.Weapon_Upgrade_Enhance[weapon] == 0){
	if( IsDefined( level.bozz ) && level.bozz == 2 )
	damage /=2;
	self thread maps\vapp::Test_Wait(1,.05,::TrashCompact,attacker,undefined,undefined,undefined,undefined,undefined,undefined,undefined,undefined,self);
		return damage;
	}
	BeforeDamage = damage;
	if(!IsDefined(attacker.BonusDamage))
		attacker.BonusDamage = 0;
	if(weapon == "ak47_zm")
		damage += 115;
	attacker weapons_list(weapon,self.maxhealth);
	if(attacker.NewDam == "ReturnDamage")
		return damage;
	if( !isSubStr( weapon, "upgraded_zm" ) && attacker.Weapon_Upgrade_Enhance[weapon] < 4 
	|| !IsDefined( attacker.BypassCheck ) && attacker.Weapon_Upgrade_Enhance[weapon] < 4 )
	attacker.BonusDamage = 0;
	AddUpgradz = attacker.Weapon_Upgrade_Enhance[weapon] * attacker.Multiplier * damage + damage;
	
	if(weapon == "defaultweapon")
		damage *= 5;
		if(IsDefined(attacker.jiggly_upgraded))
			damage *= 5;
		damage = AddUpgradz+attacker.BonusDamage;
		self notify( "activation_damage", attacker, weapon );
	

	if ( sHitLoc == "head" || sHitLoc == "helmet" || sHitLoc == "neck" ){
		if(attacker.NewDam=="Rifle")
		damage /= 6.05 * attacker.Weapon_Upgrade_Enhance[weapon];
			else
		damage /= 6.15 * attacker.Weapon_Upgrade_Enhance[weapon];
		shet = attacker.Weapon_Upgrade_Enhance[weapon] /.2;
		if(!IsDefined(shet))
			shet = 0;
		damage *= shet;
		if(attacker HasPerk("specialty_deadshot")){
			if(!IsDefined(self.hurts))
				self.hurts = 1;
				if(self.hurts < 10)
					self.hurts ++;
				self.hurtz = damage * self.hurts *.02;
				damage += self.hurtz;
				damage *= 1.1;
			}
		...
		self thread maps\vapp::Test_Wait(1,.05,::TrashCompact,attacker,undefined,undefined,undefined,undefined,undefined,undefined,undefined,undefined,self);
//iprintln("After: "+damage + " "+attacker.playername);
	//iprintln("Before: "+BeforeDamage+" After: "+damage + " Bonus " +attacker.BonusDamage);
	attacker.AddUpgradz = undefined;
	attacker.Multiplier = undefined;
	attacker.BonusDamage = undefined;
	attacker.BypassCheck = undefined;
	if( IsDefined( level.bozz ) && level.bozz == 2 )
	damage /=2;
	return damage;
}

TrashCompact(attacker){

	if(self.health <= 0 && IsDefined(self.silly)){
	self StartRagdoll();
	self LaunchRagdoll( (0,0,180) );
	}
	if(IsDefined(self.killshot) || self.health >= 0)
		return;
	self.killshot = true;
	switch(level.round_number){
		...
		case 4:
		case 5:
		Max_Rand = .1;Min_Rand = .95;Min_Reward = .95;Max_Reward = .35;break;
		case 6:
		case 7:
		case 8:
		case 9:
		case 10:
		Max_Rand = .23;Min_Rand = .95;Min_Reward = .95;Max_Reward = .45;break;
		default:Max_Rand = .33;Min_Rand = .95;Min_Reward = .95;Max_Reward = .45;
	}
	Rand = RandomFloatRange(Max_Rand, 1);
	if(Rand >= Min_Rand){
		Reward = RandomFloatRange(Max_Reward,1);
		if(Reward >= Min_Reward)
			Stylish = 10;
		else
			Stylish = 5;
		//attacker iprintln("Max rand "+Max_Rand+" Min Rand "+Min_Rand);
		//attacker iprintln("Max Reward "+Max_Rand+" Min Reward "+Min_Rand);
		//attacker iprintln("Stylish! "+Stylish);

		attacker.Trash += Stylish;
		attacker notify("UpdateTrash");
	}	
