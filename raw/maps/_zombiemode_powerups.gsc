...
set_zombie_var( "zombie_powerup_rpg_time", 30 );
set_zombie_var( "zombie_powerup_rpg_on", false );
...
level thread Power_up_hud( "zom_icon_community_pot", level.powerup_hud[4], 116, "zombie_powerup_rpg_time", "zombie_powerup_rpg_on" );
}
...
if(!isDefined(level.fireannoystop)) {
...
start_rpg( item )
{
level notify ("powerup rpg");
level endon ("powerup rpg");
level thread maps\_zombiemode_audio::do_announcer_playvox( level.devil_vox["powerup"]["rpg"] );
level.zombie_vars["zombie_powerup_rpg_on"] = true;
level.zombie_vars["zombie_powerup_rpg_time"] = 30;
while ( level.zombie_vars["zombie_powerup_rpg_time"] > 0)
{
wait(0.1);
level.zombie_vars["zombie_powerup_rpg_time"] = level.zombie_vars["zombie_powerup_rpg_time"] - 0.1;

    }
	level.zombie_vars["zombie_powerup_rpg_on"] = false;
level notify ( "rpg_off" );
}
...
{
	level notify("Ammo_Regen");
	if(!IsDefined(level.MaxInProgress)){
	level.MaxInProgress=true;
players = get_players();
for (i = 0; i < players.size; i++)
{
...
players[i].IAMWEAPON = players[i] GetCurrentWeapon();
players[i] notify( "zmb_lost_knife" );
players[i] notify( "zmb_disable_claymore_prompt" );
players[i] notify( "zmb_disable_spikemore_prompt" );
if(isDefined(level.VanillaPlus))
//level maps\vapp::dvar_all("player_ClipSizeMultiplier",255);
for( x = 0; x < primary_weapons.size; x++ )
{
	level.WeaponSize = x;
	//iprintln(primary_weapons[x]);
if(!isDefined(level.VanillaPlus)){
players[i] GiveMaxAmmo( primary_weapons[x] );
players[i] setWeaponAmmoClip( primary_weapons[x],999 );
if(x <= 1)
players[i] SwitchToWeapon( primary_weapons[x] );wait .005;
 if( primary_weapons[x] == "defaultweapon")
	 players[i] maps\VapAmmo::WeaponRefill(primary_weapons[x]);
} else{
players[i] maps\VapAmmo::WeaponRefill(primary_weapons[x]);wait .005;
}
}
if(isDefined(level.VanillaPlus)){
level maps\vapp::dvar_all("player_ClipSizeMultiplier",level.ClipSize);
}
level thread full_ammo_on_hud( drop_item );
level.MaxInProgress = undefined;
}/*else{
iprintln("Prevented Ammo Bug");
}*/
}
}
...
ent_player iprintln( "Hold Sprint To Switch Out" );
ent_player thread minugun_switch_checker();
}
...
if( IsDefined( level.VanillaPlus ) )
{
	ent_player thread maps\vapp::Minigun_Powerup("minigun_zm", 175, "player_downed");
	wait .1;
	ent_player.zombie_vars["zombie_powerup_minigun_time"] = 1;
}

}
minugun_switch_checker(){
	self thread minigun_hud_checker();
self endon("swap");
timer = 0;
for(;;){
if(self SprintButtonPressed()){
	if(!IsDefined(self.MiniOn)) {
		if( !isdefined(self.MiniProgressBar) ) {
		self.MiniProgressBar = self maps\_hud_util::createPrimaryProgressBar();
	}
	self.MiniOn = true;
	self.MiniTextHud.alignX = "center";
	self.MiniTextHud.alignY = "middle";
	self.MiniTextHud.horzAlign = "center";
	self.MiniTextHud.vertAlign = "bottom";
	self.MiniTextHud.y = -113;
	self.MiniTextHud.foreground = true;
	self.MiniTextHud.font = "default";
	self.MiniTextHud.fontScale = 1.8;
	self.MiniTextHud.alpha = 1;
	self.MiniTextHud.color = ( 1.0, 1.0, 1.0 );
	self.MiniTextHud setText( "Canceling Death Machine" );
	self.MiniProgressBar maps\_hud_util::updateBar( 0.01, 1 / 1.3);
	self thread minigun_hud_checker();
	}
wait .05;
timer += .05;
if(timer >= 1.3){
	self.zombie_vars["zombie_powerup_minigun_time"] = 1;
		self iprintln("Swapped");
		wait 1.1;
	self notify("swap");
	}
} else {
...
self.MiniOn = undefined;
wait .05;
timer = 0;
self notify("MiniCancel");
}
}
}
minigun_hud_checker(){
self waittill_any("death","swap","disconnect","player_downed","MiniCancel");
if( isdefined(self.MiniProgressBar) )
self.MiniProgressBar maps\_hud_util::destroyElem();
}
...
if( IsDefined( level.VanillaPlus ) )
{
players[i].Trash += 10 + level.round_number;
players[i] notify("UpdateTrash");
}
}
carp_ent delete();
}
