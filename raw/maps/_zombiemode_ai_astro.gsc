...
init()
{
	...
	level.astro_zombie_health_mult = 5;
	level.min_astro_round_wait = 2;
	level.max_astro_round_wait = 4;
	level.astro_round_start = 9;
...
astro_prespawn()
{
	self.astro_headbutt_delay = 2000;
...
{
	self endon( "death" );
	self thread Astro_Round_Watcher();
	self thread Astro_Health_Watcher();
	self.is_headbutt = false;
	self.next_headbutt_time = GetTime() + self.astro_headbutt_delay;
	...
	while ( 1 )
	{
		...
		if ( !self.is_headbutt && GetTime() > self.next_headbutt_time )
		{
			...
			if(is_true( self.player_to_headbutt.jetpack ) )
				headbutt_anim = %ai_zombie_taunts_7;
			else
...
astro_zombie_headbutt_watcher( animname )
{
	...
	while ( 1 )
	{
		...
		if ( !isdefined( self.player_to_headbutt ) || !is_player_valid( self.player_to_headbutt ) || is_true( self.player_to_headbutt.jetpack ) )
		{
			continue;
		}
...
Astro_Speed()
{
	if( self.health <= self.maxhealth / 2.5 || self.rounds == 5 )
	{
	...
	if( self.health <= self.maxhealth / 2.2 || self.rounds == 4 )
	{
		...
		if( self.HasUpdate >= 4)
			return;
		self.HasUpdate = 4;
		self.astro_headbutt_delay += 4000;
		self.moveplaybackrate += .05;
		return;
	}
	
	if( self.health <= self.maxhealth / 1.9 || self.rounds == 3 )
	{
	...
	if( self.health <= self.maxhealth / 1.6 || self.rounds == 2 )
	{
	...
	if( self.health <= self.maxhealth / 1.3 || self.rounds == 1 )
	{
		...
		if( self.HasUpdate >= 1)
			return;
		self.HasUpdate = 1;
		self.astro_headbutt_delay += 1250;
		return;
	}
}
...
{
	self endon( "death" );
	self endon( "max_speed" );
	self SetHUDWarningType( "zombie_friend" );
	self.IgnoreSpecialDamage = true;
	pp_anim = %ai_zombie_taunts_4;
	self thread maps\_zombiemode_spawner::zombie_eye_glow_stop();
	self.prevh = undefined;
	//wait 3;
	//iprintln("Name Changed");
	self setzombiename( level.zomspace );
	if( !IsDefined( self.rounds ) )
		self.rounds = 0;
	self.moveplaybackrate += .1;
	for(;;)
	{
		...
		self.moveplaybackrate += .1;
		self.prevh = self.health;
		self.health = 9999999;
		self.rounds ++;
		self astro_animscripted( pp_anim, "director_anim" );
		//iprintln( "Old Man " + self.rounds );
		self.health = self.prevh;
		if( self.rounds == 6 )
			self notify("max_speed");
	}
	
}


astro_animscripted( director_anim, director_notify, finish_anim )
{
