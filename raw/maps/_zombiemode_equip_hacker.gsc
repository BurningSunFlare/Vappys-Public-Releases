			...
			for(i = 0; i < players.size; i ++)
			{
				if(IsDefined(players[i] get_player_equipment()) && players[i] get_player_equipment() == "equip_hacker_zm")
				{
					...
					if(IsDefined(players[i].equipment_got_in_round["equip_hacker_zm"]))
					{
						...
						if(rounds_kept > 0)
						{
							rounds_kept = min(rounds_kept, 4);
							score = rounds_kept * 230;
							if(IsDefined( level.VanillaPlus ) )
							{
								score = rounds_kept * 190;
								score = score / Divider;
							}
...
hacker_do_hack( hackable )
{
	...
	hack_duration *= 0.475;
	if(self hasperk("specialty_fastreload") && IsDefined(level.fast_hacked))
		hack_duration *= 0.475;
	...
	self.hackerTextHud.color = ( 1, 1, 0 );
