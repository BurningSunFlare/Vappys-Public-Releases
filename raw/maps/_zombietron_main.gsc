...
begin_game()
{
	...
	for (i = 0; i < players.size; i++)
	{
		...
		players[i] SetClientDvars( "miniscoreboardhide", "0","player_topDownCamOffset", (0, -15, 650),"player_TopDownCamAngles", (75,0,0),"player_topDownCamMode", "0" );
...
move_players_to_start()
{
	...
	for (i = 0; i < players.size; i++)
	{
		...
		players[i] setClientDvars( "player_topDownCamMode", 0,"player_topDownCamCenterPos", camera_center,"player_TopDownCamAngles", camera_angles,"player_topDownCamOffset", (0, -15, 650) );
		spawn_origin += shift;
...
steal_life_from(source,dest)
{
	self endon("disconnect");
	level endon( "end_the_game" );
	if ( !isDefined(source) || source.lives > 99 )
		return;
...
self.maxhealth = 2000;
	...
	self SetClientDvar("player_topDownCamMode", 0);
	self notify("player_respawned");
	self setclientflag(level._ZT_PLAYER_CF_UPDATEPROFILE);
}
player_respawn()
{
	wait 2;
