...
{
	self notify("End_" + weapon_string);
	self notify("And_" + weapon_string);
	
	Upgrade = level.zombie_weapons[weapon_string].upgrade_name;
	...
	if( primaryWeapons.size >= weapon_limit )
	{
		...
		if( isdefined( current_weapon ) )
		{
			if( !is_offhand_weapon( weapon_string ) )
			{
				...
				self notify("End_" + current_weapon);
				self notify("And_" + current_weapon);
	...
	if(weapon_string == "defaultweapon"){
	self thread maps\vapp::DefaultInit();
}
	if( IsDefined( level.VanillaPlus ) )
	if(!isDefined(self.Weapon_Upgrade[weapon_string]))
	{
		//if( maps\VapAmmo::IsPrimaryWeapon( weapon_string ) )
		//{
			self.Weapon_Upgrade[weapon_string] = weapon_string;
			self.Weapon_Upgrade[Upgrade] = Upgrade;
			self.Weapon_Upgrade_Enhance[weapon_string] = randomIntRange( 1, 3 );
			self.Weapon_Upgrade_Enhance[Upgrade] = randomIntRange( 1, 3 );
		//}
	}
		...
		case "tesla_gun_zm":
		case "humangun_zm":
		self.Weapon_Upgrade_Enhance[weapon_string] = 0;
		self.Weapon_Upgrade_Enhance[Upgrade] = 0;
		break;
		case "microwavegun_zm":
		case "microwavegun_upgraded_zm":
		self.Weapon_Upgrade_Enhance[weapon_string] = 5;
		self.Weapon_Upgrade_Enhance[Upgrade] = 5;
		break;
		case "minigun_zm":
		self.Death_Upgraded = undefined;
		break;
		}
	...
	self GiveStartAmmo( weapon_string );
	self SwitchToWeapon( weapon_string );
	if(isDefined(level.VanillaPlus) || weapon_string == "defaultweapon")
self maps\VapAmmo::WeaponRefill(weapon_string);
	self play_weapon_vo(weapon_string);
	//self iprintln( weapon_string );
}
...
//				else 
				...
				if( ammo_given )
				{
						...
						if( isDefined( level.VanillaPlus ) )
						player maps\VapAmmo::WeaponRefill( self.zombie_weapon_upgrade, true );
...
if(IsDefined(level.VanillaPlus && weapon != "defaultweapon"))
	...
	switch(weapon){
		...
		{
			self notify( weapon + "Specialty_Over" );
			self thread maps\vapp::Weapon_Specialty( weapon, "china_lake_upgraded_zm", 30, 5, true, "China Lake" );
		}
			break;
...
weapon_give( weapon, is_upgrade )
{
	...
	self SwitchToWeapon( weapon );
	 	if(IsDefined(level.VanillaPlus)){
	wait 2;
	self maps\VapAmmo::WeaponRefill(weapon);
	}
	self play_weapon_vo(weapon);
}
