...
{
	ent iprintln("somethings in");
	if(ent.IgnoreGrav == false || ent.IgnoreGrav == undefined && ent.IgnoreGrav != true)
	ent setplayergravity( 136 );
}
gravity_player_out( ent )
{
	ent iprintln("somethings out");
	if(ent.IgnoreGrav == false || ent.IgnoreGrav == undefined && ent.IgnoreGrav != true)
...
gravity_zombie_update( low_gravity, force_update )
{
	...
	if(self.is_slowed == true)
	self waittill("ILostFreezeEffect");
	...
	while ( 1 )
	{
		...
		for ( i = 0; i < player_zones.size; i++ )
		{
			...
			if ( isdefined( zone ) && is_true( zone.is_enabled ) )
			{
				...
				{
					player = players[j];
					if(player.IgnoreGrav == false || player.IgnoreGrav == undefined && player.IgnoreGrav != true) {
					...
					if ( is_player_valid( player ) && player istouching( volume ) )
					{
						...
						} else if(isDefined(player.swapped)&& volume.script_string != "lowgravity"){
							player setplayergravity( LOW_G );
							player.in_low_gravity = true;
							}
						else if ( isdefined( volume.script_string ) && volume.script_string == "lowgravity" &&!isdefined(player.swapped))
						{
							player setplayergravity( LOW_G );
							player.in_low_gravity = true;
						}
...
low_gravity_watch()
{
	...
	while ( 1 )
	{
		...
		if ( ( !flag( "power_on" ) || is_true( self.in_low_gravity ) ) && !self maps\_zombiemode_equip_gasmask::gasmask_active() )
		{
			...
			if ( self HasPerk( "specialty_longersprint") && !IsDefined(level.sour_hacked) )
				time_to_death = time_to_death_jug;
			else if(self HasPerk("specialty_longersprint") && IsDefined(level.sour_hacked))
			time_to_death = time_to_death_sour;
			else
			{
			...
			if ( time_in_low_gravity > time_to_death )
			{
				...
				if(self.has_starlight == true && self.health > 5)
				self DoDamage( self.health / 6, self.origin );
				else
